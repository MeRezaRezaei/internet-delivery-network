...
class RiskGuard
{
    ...
    protected const OUT_OF_SCOPE_NODES = ['srv02', 'srv08', 'srv10'];
    ...
    public function validateNodeAccess(Node $node): void
    {
        ...
        if (in_array($node->name, self::OUT_OF_SCOPE_NODES)) {
            throw new Exception("MVP BOUNDARY VIOLATION: Access to {$node->name} is out of scope for MVP.");
        }
    }
