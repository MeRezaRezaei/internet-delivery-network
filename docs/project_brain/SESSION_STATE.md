...
## Done
- **Multicast HAProxy Generator Alignment (2026-05-22):** Realigned the dynamic HAProxy configuration compiler script (`scripts/generate_haproxy.py`) to match the user's exact inventory mapping (3 outside servers `01`-`03`, 6 inside servers `01`-`06`, 6 CDNs, and tunnel ID `05`). Successfully generated custom configs for all 6 target inside nodes mapping the WireGuard private IP subnet `10.255.1.1` through `10.255.1.6`, achieving fully decentralized multicast mesh routing with zero-touch management safety.
