...
def run_local(cmd):
    # Runs command on local Windows host
    res = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    return res.returncode, res.stdout, res.stderr

def run_remote_srv04(cmd):
    # Runs command on Server 04 via SSH over Server 07
    full_cmd = f'{SSH_BASE} "{JUMP_SSH} \'{cmd}\'"'
    res = subprocess.run(full_cmd, shell=True, capture_output=True, text=True)
    return res.returncode, res.stdout, res.stderr

def upload_configs():
    # Copy local generated files to Server 07, then to Server 04
...
def restart_xray():
    # Stop and start xray processes on Server 04
...
def run_benchmark():
    ...
    for i in range(5):
        ...
        # Fetch google.com headers over SOCKS port 10800
        ret, stdout, stderr = run_remote_srv04("curl -s -o /dev/null -w '%{http_code}' --socks5-hostname 127.0.0.1:10800 https://www.google.com --max-time 10")
        end_time = time.time()
        duration = end_time - start_time
        
        http_code = stdout.strip()
        if ret == 0 and http_code == "200":
...
def main():
    print("Starting loopback benchmark across parameter sets...")
    ...
    for name, params in SCENARIOS.items():
        ...
        print(f"Result for {name}: Successes: {successes}/5, Avg Latency: {avg_latency:.3f}s")
