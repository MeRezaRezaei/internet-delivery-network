...
# CONFIG GENERATOR
# ===================================================================
def generate_unified_xray(outside_servers, inside_servers, cdns):
    combinations = []
    for t in TUNNEL_IDS:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dynamic Xray Config Generator")
    ...
    parser.add_argument("--cdns", type=str, default="01,05", help="Comma-separated list of active CDN servers")
    args = parser.parse_args()

    out_dir = os.path.abspath("configs/xray/generated")
    os.makedirs(out_dir, exist_ok=True)
    
    outside_servers = [x.strip() for x in args.outside.split(",") if x.strip()]
    inside_servers = [x.strip() for x in args.inside.split(",") if x.strip()]
    cdns = [x.strip() for x in args.cdns.split(",") if x.strip()]

    config, excludes = generate_unified_xray(outside_servers, inside_servers, cdns)
