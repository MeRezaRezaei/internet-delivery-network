...
|---|---|---|
| Runtime states canonical and mapped end-to-end | pass | `idn:fleet:reconcile` command implemented to sync DB/Redis, tested in `GapRecoveryTest` |
...
| MVP out-of-scope boundaries enforced | pass | `RiskGuard::OUT_OF_SCOPE_NODES` enforced, verified in `RiskGuardTest::test_risk_guard_enforces_mvp_out_of_scope_boundaries` |
...
| Gap-recovery behavior defined and tested | pass | `GapRecoveryTest.php` defines recovery flows for missing and unsynced nodes using `idn:fleet:reconcile` |
