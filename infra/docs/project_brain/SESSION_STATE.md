...
- **Laravel Project Initialization & IDN Integration (2026-05-27):**
    ...
    - Created `IDNBaseCommand` to wrap and execute `infra/scripts` safely.
...
- **Server 04 Local Loopback Tunnel Connection Success (2026-05-26) [VERIFIED ON SERVER 04]:**
    ...
    - Discovered and resolved a critical port clash on Server 04: a zombie Xray process (`474540`) and the active Marzban Node container (`node_node_1`) were actively clashing on port `8443`, triggering connection failures and sporadic `unexpected status 400` errors on XHTTP.
    - Temporarily stopped the `node_node_1` container and killed all duplicate/stray Xray processes on Server 04 to clean the slate and leave port `8443` completely clear.
    - Relaunched the Portal and Bridge loopback configs in the background on Server 04, achieving **100% active reverse tunnel registration** of `v1.rvs.cool:0` and `udp:reverse:0` over XHTTP H2 `packet-up` mode.
    - Verified **successful tunnel loopback routing to domestic target**: Sent a test request `curl --socks5-hostname 127.0.0.1:10800 https://iran.ir` (routing SOCKS request to Portal -> detoured to reverse tunnel -> Bridge -> freedom direct outbound to domestic IP) which successfully resolved, dialed, and returned the actual HTML response!
