...
- **Custom XHTTP Subscription Service Implementation**:
    ...
    - **Database Integration**: Configured a secondary `marzban` database connection in `config/database.php` and `.env` to pull data from the baremetal Marzban MySQL.
    - **Connectivity**: Enabled `host.docker.internal` in `docker-compose.yml` for container-to-host MySQL communication.
    - **Models**: Developed Eloquent models (`Marzban\User`, `Marzban\Proxy`) with appropriate connection and table bindings.
    - **Core Logic**: Implemented `MarzbanSubscriptionController` capable of parsing Marzban tokens (username-based) and returning Base64-encoded VLESS lists.
    - **XHTTP Templates**: Created `config/marzban_sub.php` containing the exact JSON `extra` structures for direct and reverse SplitHTTP configurations as requested by the user.
    - **Protocol specifications**: Documented v2rayN/v2rayNG subscription formats in `infra/docs/SUBSCRIPTION_SPECIFICATION.md`.
