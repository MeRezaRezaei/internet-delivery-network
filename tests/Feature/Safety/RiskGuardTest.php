...
class RiskGuardTest extends TestCase
{
    ...
    public function test_risk_guard_enforces_mvp_out_of_scope_boundaries()
    {
        $riskGuard = new RiskGuard();

        $outOfScopeNodes = ['srv02', 'srv08', 'srv10'];

        foreach ($outOfScopeNodes as $nodeName) {
            $node = Node::factory()->create(['name' => $nodeName]);
            
            try {
                $riskGuard->validateNodeAccess($node);
                $this->fail("Expected exception for out-of-scope node {$nodeName}");
            } catch (Exception $e) {
                $this->assertEquals("MVP BOUNDARY VIOLATION: Access to {$nodeName} is out of scope for MVP.", $e->getMessage());
            }
        }
    }
}
